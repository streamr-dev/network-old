...
export class WebRtcEndpoint extends EventEmitter implements IWebRtcEndpoint {
    ...
    async connect(
        ...
        connection.once('reconnectionRequired', (originatorPeerInfo, routerId, type, description) =>
            this.reconnectionRequired(originatorPeerInfo, routerId, type, description)
        )
    ...
    close(receiverNodeId: string, reason: string): void {
        ...
        if (connection) {
            ...
            this.logger.warn(receiverNodeId, 'closed already')
        }
    }
