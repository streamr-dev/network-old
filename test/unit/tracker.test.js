const assert = require('assert')
const Tracker = require('../../src/logic/Tracker')
const Connection = require('../../src/connection/Connection')

describe('tracker creation', () => {
    it('should be able to start and stop successfully', (done) => {
        const PRIVATE_KEY = ''

        const connection = new Connection('127.0.0.1', 30335, PRIVATE_KEY)
        const tracker = new Tracker(connection)

        tracker.connection.on('node:ready', () => {
            tracker.connection.node.peerInfo.multiaddrs.forEach((ma) => {
                assert.equal(
                    ma.toString(),
                    '/ip4/127.0.0.1/tcp/30335/ipfs/QmQ2zigjQikYnyYUSXZydNXrDRhBut2mubwJBaLXobMt3A'
                )
            })

            // then stop tracker
            tracker.connection.node.stop(() => done())
        })
    })
})
