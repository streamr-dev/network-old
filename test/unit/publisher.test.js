const assert = require('assert')
const Publisher = require('../../src/logic/Publisher')
const Connection = require('../../src/connection/Connection')

describe('publisher creation', () => {
    it('should be able to start and stop successfully', (done) => {
        const PRIVATE_KEY = ''

        const connection = new Connection('127.0.0.1', 30334, PRIVATE_KEY, true)
        const publisher = new Publisher(connection)

        publisher.connection.on('node:ready', () => {
            publisher.connection.node.peerInfo.multiaddrs.forEach((ma) => {
                assert.equal(
                    ma.toString(),
                    '/ip4/127.0.0.1/tcp/30334/ipfs/QmQ2zigjQikYnyYUSXZydNXrDRhBut2mubwJBaLXobMt3A'
                )
            })

            // then stop tracker
            publisher.connection.node.stop(() => done())
        })
    })
})
