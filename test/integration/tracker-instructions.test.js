...
describe('check tracker, nodes and statuses from nodes', () => {
    ...
    it('if failed to follow tracker instructions, inform tracker about current status', async () => {
        ...
        await node1.onTrackerInstructionReceived(encoder.decode('tracker', trackerInstruction))
        await node2.onTrackerInstructionReceived(encoder.decode('tracker', trackerInstruction))
